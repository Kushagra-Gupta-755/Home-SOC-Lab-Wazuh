...
- **SIEM Deployment:** Deployment of the Wazuh SIEM platform to provide centralized log monitoring and security event analysis.

- **Endpoint Monitoring:** The Windows endpoint is monitored using the Wazuh agent, which collects system logs and sends them to the SIEM server.

- **File Integrity Monitoring:** Real-time monitoring of selected directories to detect file creation, modification, and deletion events.

- **Windows Log Monitoring:** Collection and analysis of important Windows logs including: Security logs, System logs, Application logs, PowerShell operational logs, Windows firewall logs

- **Network Scan Detection:** Network reconnaissance activity was simulated using Nmap from the Kali Linux attacker machine. Wazuh detected repeated firewall drop events generated by the scan.

- **Vulnerability Detection:** Wazuh scans installed software and identifies known vulnerabilities using CVE databases.

- **Threat Hunting:** Security alerts can be investigated using the Wazuh dashboard to analyze log data, rule triggers, and event details.
